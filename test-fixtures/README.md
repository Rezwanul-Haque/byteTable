...
Throwaway databases for exercising all four engines. **Test data only — never production.**

## Bring it up

```sh
cd test-fixtures
docker compose up -d        # Postgres + MySQL + Redis (Postgres/MySQL auto-seed on first init)
./seed/seed-redis.sh        # seed Redis (no auto-init dir for Redis)
docker compose down -v      # stop + wipe volumes (next `up` re-seeds)
```

Ports are offset (5**5432**/3**3306**/6**3790**) so they won't collide with any local Postgres/MySQL/Redis.
...
- `seed/seed-redis.sh` — run manually after `up`.
...
While testing M14, the same three containers (`bt-pg`/`bt-mysql`/`bt-redis`) are likely already up from an ad-hoc launch with identical credentials. If `docker compose up` reports a port/name conflict, remove the ad-hoc ones first: `docker rm -f bt-pg bt-mysql bt-redis`, then `docker compose up -d`.

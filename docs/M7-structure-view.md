...
- **`ColumnInfo`** (`engine.rs:517`) — `name`, `data_type` (declared text, may be empty), `nullable`, `pk` (every member of a composite pk), `default_value` (wire name `default`, verbatim DEFAULT text or `null`), `fk: Option<FkRef>`, `auto_increment` (engine-assigned on insert: MySQL `AUTO_INCREMENT`, Postgres identity/`serial`, SQL Server `IDENTITY`, SQLite's `INTEGER PRIMARY KEY` rowid alias; read-only, shown as the "AUTO INC" badge). **`FkRef`** (`engine.rs:543`) = `{ table, column }` (empty `column` = unresolvable implicit target).

...
/// Postgres's 65535 bind-parameter ceiling. Any error rolls the whole call
/// back. NULL JSON binds as SQL NULL (`BoundValue::from_json_set`).
async fn bulk_insert(
    pool: &PgPool,
    schema: &str,
    table: &str,
...
async fn delete_rows(pool: &PgPool, req: &DeleteRowsRequest) -> Result<DeleteRowsResult, AppError> {

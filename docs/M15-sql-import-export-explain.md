...
- `export_table(manager, handle, schema, table, format) -> Result<String>` — dispatches to:
  ...
  - `export_table_sql` — `table_meta.ddl` (terminated with `;` if it does not already end in one, so the dump re-imports cleanly) + a blank line + `INSERT INTO {q(table)} ({quoted cols}) VALUES (…);` per row; `-- (no rows)` when empty.
  - **Schema portability**: table names in a dump are **unqualified** — both the `CREATE TABLE` (Postgres's assembled DDL has its `{q(schema)}.` prefix stripped by `unqualify_create`) and the `INSERT`s — so the import session decides the destination (`execute_script` runs `USE` / `SET search_path` first). Qualifying with the SOURCE schema sent rows back to the table they were read from, surfacing as a duplicate-key error instead of an import. `EngineConnection::dump_qualifies_schema` opts out the engines with no session schema to point (MSSQL: no `USE <schema>`; ClickHouse: database comes from the connection), whose dumps keep `{q(schema)}.{q(table)}` and restore only into their source schema.
...
- **Export flow** (`features/export/exportFlow.ts`) — `runExport`: generate text via the backend **first** (so a §5 surfaces before the file picker) → native `save()` dialog (default name per kind, each ending in the `exportStamp()` tail `_{env}_{YYYYMMDD-HHmmss}` in local time — `{table}_production_{stamp}.csv`, `{table}_production_{stamp}.sql`, `{schema}_schema_production_{stamp}.sql` — so a dump says which deployment it came from and repeated dumps never collide; env omitted when unknown) → `exportSave` → `toast("Exported {file}", "ok")`. Cancelled dialog = silent; dialog plugin missing (browser dev) = `toast("Export requires the desktop app", "info")`.
...
- **Export table SQL** — "Export as SQL (schema + data)" produces the `CREATE TABLE …;` DDL + one `INSERT INTO "table" (…) VALUES (…);` per row (unqualified, so the dump imports into any schema); `'` doubled in string literals; `-- (no rows)` for an empty table.

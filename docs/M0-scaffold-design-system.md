...
Only the M0-relevant subset is required to *build M0*; the rest were added by later milestones (M2/M12/M13) but the spec lists what is in the shipped tree so a rebuild compiles against the same lock.
...
export interface Preferences { accent: Accent; darkness: Darkness; density: Density }
export const defaultPreferences: Preferences = { accent:"teal", darkness:"charcoal", density:"compact" };
